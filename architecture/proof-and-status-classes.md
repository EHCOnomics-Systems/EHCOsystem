...
The EHCOsystem keeps Runtime, artifact, declaration, test, observation, deployment, evidence, publication, and release states distinct. No class may be promoted into another without an applicable governing record.

## Runtime classes

### Runtime realized

EHCO AI-OS exists as the realized Tier 1 Runtime. Runtime realization is not created by a repository, source folder, build, test, packet, container, dashboard, or public document.

### Runtime standing

The accepted standing baseline is **52/53**. Standing changes only through the applicable Runtime authority and proof path. Repository activity, packet verification, assistant interpretation, or scoped absence does not independently change standing.

### Runtime admitted

A participant, component, or operation has entered the Runtime through the governing admission and binding relationships.
...
An exact file, folder snapshot, manifest, image, package, or document is identified by path, revision, hash, byte count, or another declared identity control. Artifact identity is not Runtime identity.
...
Scope miss is not state change. Ledger is evidence, not authority. Dashboard is projection, not Runtime truth. Bind claims to proof roots before declaring gaps.
