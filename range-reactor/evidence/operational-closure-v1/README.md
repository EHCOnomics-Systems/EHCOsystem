...
The accepted benchmark is **physical Docker/Linux execution evidence on the owning Windows/Docker estate**, not simulation-only or CI-only evidence. The matched A/B was executed through that owning estate in a controlled Docker/Linux environment using CPython 3.11.16, with exact benchmark source bound to `e72b2a29e52878d300b44f0286259466352f73cc` and the retained physical result file independently hashed.

Range Reactor also has a separate, earlier **real-world container/service qualification** bound to owning source revision `149314be7c4302d66bd93ac112e062292a2e815b` and immutable qualification image `sha256:f69cd78902d2a8b97e4f70f86b779159dac394c9d43ddcb81611a76897055c0a`. That qualification exercised the software as a containerized internal HTTP service and recorded PASS for source/static contract, build identity, private-network health, capability execution, fail-closed transport handling, timeout/unavailability behavior, restart recovery, temporary portability, and AI-OS fail-closed fallback. Diagnostic service performance was also collected, but it had no acceptance threshold and is not substituted for the later exploration-algorithm A/B result.
