#!/usr/bin/env python3
"""Validate the public dossier and Public Evidence Companion repository estate.

This validator uses only the Python standard library. Its result is bounded to
repository identity, structure, manifest integrity, JSON syntax, navigation,
and high-confidence secret-indicator checks.
"""
...
def validate_repository_residue() -> None:
    ...
    for directory in forbidden_dirs:
        checked(not directory.exists(), f"Forbidden directory present: {directory.relative_to(REPO_ROOT)}")

    wrong_pdf = REPO_ROOT / "evidence" / "public-evidence-companion" / DOSSIER_NAME
    checked(not wrong_pdf.exists(), f"Legacy PDF path present: {wrong_pdf.relative_to(REPO_ROOT)}")
    ...
    for path in REPO_ROOT.rglob("*"):
        ...
        if path.is_file():
            ...
            checked(not re.fullmatch(r"part-\d+", path.name), f"Transfer chunk present: {relative}")
...
def validate_dossier() -> None:
    ...
    for path in paths:
        ...
        if path.is_file():
            ...
            checked(digest == DOSSIER_SHA256, f"Dossier SHA-256 mismatch: {path.relative_to(REPO_ROOT)}")
    if len(hashes) == 2:
...
def validate_packets_and_manifests() -> dict[str, str]:
    ...
    actual_dirs = sorted(path.name for path in EVIDENCE_ROOT.iterdir() if path.is_dir()) if EVIDENCE_ROOT.is_dir() else []
    ...
    for packet_name in EXPECTED_PACKETS:
        ...
        for entry in files:
            ...
            if target.is_file():
                checked(target.stat().st_size == entry.get("bytes"), f"Byte-count mismatch: {target.relative_to(REPO_ROOT)}")
                checked(sha256_file(target) == str(entry.get("sha256", "")).upper(), f"SHA-256 mismatch: {target.relative_to(REPO_ROOT)}")
        ...
        if verification_path.is_file():
            ...
            if isinstance(verification, dict):
                checked(verification.get("status") == "PASS", f"Verification status is not PASS: {packet_name}")
...
def validate_suite(manifest_hashes: dict[str, str]) -> None:
    ...
    checked(suite.get("packets_scope") == "PRIOR_PACKETS_00_THROUGH_07", "Unexpected suite packet scope")
    ...
    if isinstance(packets, list):
        for index, entry in enumerate(packets):
            ...
            checked(str(entry.get("manifest_sha256", "")).upper() == manifest_hashes.get(expected), f"Suite manifest hash mismatch for {expected}")
            checked(entry.get("verification_status") == "PASS", f"Suite verification status is not PASS for {expected}")

    closure = suite.get("closure_packet", {})
    checked(isinstance(closure, dict) and closure.get("sequence") == 8, "Closure packet sequence must be 8")
    checked(isinstance(closure, dict) and closure.get("directory") == EXPECTED_PACKETS[8], "Closure packet directory mismatch")
    checked(isinstance(closure, dict) and closure.get("verification_status") == "PASS", "Closure packet verification is not PASS")
...
def validate_markdown_links() -> None:
    ...
    for markdown in sorted(REPO_ROOT.rglob("*.md")):
        ...
        for raw_target in link_pattern.findall(text):
            ...
            checked(resolved.exists(), f"Broken Markdown link in {markdown.relative_to(REPO_ROOT)}: {raw_target}")
...
def validate_secret_indicators() -> None:
    ...
    for path in REPO_ROOT.rglob("*"):
        ...
        for label, pattern in patterns.items():
            checked(pattern.search(text) is None, f"High-confidence {label} indicator in {path.relative_to(REPO_ROOT)}")
...
def main() -> int:
    ...
    print(
        "PASS: public evidence repository validation "
        f"({CHECKS} checks, {file_count} evidence files, {json_count} JSON files, "
        f"{len(EXPECTED_PACKETS)} packet directories)"
    )
    return 0
